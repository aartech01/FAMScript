...
function _preprocessMermaid(raw) {
  ...
  const cleanLines = text.split('\n').filter(line => {
    ...
    if (cm) { coupleIds.push(cm[1]); return true; }
...
function log(entries) {
  consoleEl.innerHTML = entries.map(e => {
    ...
    return `<div class="log-line log-${e.type || 'info'}">${icon} ${escHtml(e.message)}</div>`;
  }).join('');
}
...
/* ═══════════════ EVENT LISTENERS ═══════════════ */
editor.addEventListener('input', () => { setStatus('idle'); });
